...
app.post('/search', (req, res) => {
  ...
  query(connection, 'SELECT * FROM shipped_package WHERE package_number = ?', [req.query.number])
    .then((results) => {
...
app.post('/packages', verifyUser, (req, res) => {
  ...
  query(connection, 'INSERT INTO shipped_Package (package_number, weight, height, insurance_amount, destination, final_date,length,width,type,owned_by,package_status,received_by) VALUES (?, ?, ?, ?, ?, ?,?,?,?,?,?,?)', [req.body.number, req.body.weight, req.body.height, req.body.insurance, req.body.destination, req.body.date,req.body.length,req.body.width,req.body.type,1,req.body.status,req.body.received_by])
    .then(() => {
...
app.delete('/packages/:number', verifyUser, (req, res) => {
  ...
  query(connection, 'DELETE FROM shipped_Package WHERE package_number = ?', [req.params.number])
    .then(() => {
      // Return a success message if the query was successful
...
app.put('/packages/:number', verifyUser, (req, res) => {
  ...
  query(connection, 'UPDATE shipped_Package SET weight = ?, dimensions = ?, insurance_amount = ?, destination = ?, final_delivery_date = ? WHERE package_number = ?', [req.body.weight, req.body.dimensions, req.body.insurance, req.body.destination, req.body.date, req.params.number])
    .then(() => {
      // Return a success message if the query was successful
...
app.put('/users/:id', verifyUser, (req, res) => {
  ...
  query(connection, 'UPDATE Users SET name = ?, email = ? WHERE id = ?', [req.body.name, req.body.email, req.params.id])
    .then(() => {
      // Return a success message if the query was successful
...
app.get('/report', verifyUser, (req, res) => {
  ...
  query(connection, 'SELECT * FROM shipped_Package WHERE (package_status = ?) AND Final_Date BETWEEN ? AND ?', [req.query.status,req.query.start, req.query.end])
    .then((results) => {
...
app.get('/trace/:number', verifyUser, (req, res) => {
  ...
  query(connection, 'SELECT * FROM package_locations WHERE package_number = ?', [req.params.number])
    .then((results) => {
      // Return the results to the user
      res.send({ results });
    })
...
app.post('/login',urlencodedParser, (req, res) => {
  ...
  db.query(connection, 'SELECT * FROM Users WHERE email = ? AND password = ?', [req.body.email, req.body.password])
    .then((results) => {
...
app.get('/packages/:number/update', verifyUser, (req, res) => {
  ...
  db.query(connection, 'SELECT * FROM shipped_package WHERE package_number = ?', [req.params.number])
    .then((results) => {
...
app.post('/packages/:number/update', verifyUser, (req, res) => {
  ...
  db.query(connection, 'UPDATE shipped_package SET weight = ?, height = ?, insurance_amount = ?, destination = ?, final_date = ? WHERE package_number = ?', [req.body.weight, req.body.height, req.body.insurance, req.body.destination, req.body.date, req.params.number])
    .then(() => {
    ...
    .catch((error) => {
      ...
      res.redirect('/packages/' + req.params.number + '?error=' + encodeURIComponent(error.message));
    });
});
...
app.post("/report",(req,res)=>{ 
  ...
  if (req.query.status) {
   ...
   .then((results) => {
     ...
     const totalWeight = results.reduce((sum, package) => sum + package.weight, 0);
 
     // Calculate the total insurance amount of the packages
     const totalInsurance = results.reduce((sum, package) => sum + package.insurance_amount, 0);
 
     // Calculate the total value of the packages
     const totalValue = results.reduce((sum, package) => sum + package.value, 0);
     ...
     const totalFines = results.filter((package) => package.status === 'delayed').reduce((sum, package) => sum + package.fines, 0);
  ...
  db.query(connection, 'SELECT * FROM Users WHERE email = ?', [req.body.email])
    .then((results) => {
      ...
      if (results.length > 0) {
        ...
        res.redirect('/?error=' + encodeURIComponent('The user with the specified email address was not found'));
      }
    })
    .catch((error) => {
      // Redirect the user to the home page with an error message
